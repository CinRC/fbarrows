% !TeX document-id = {0f633dbb-6d1c-49f5-b754-fb8660acacac}
% !TeX TS-program = pdflatex

\documentclass{scrartcl} % We use the "koma-script article" class ( https://www.ctan.org/pkg/scrartcl )
\usepackage{fbarrows}

% \tikzset{>={Computer Modern Rightarrow}} % Have all the arrow tips match the tips of CM. 
\usepackage{csquotes}
\usetikzlibrary{arrows}  % Required for the tiKz examples below.

\title{fbarrows -- A package to draw \enquote{forward} and \enquote{backward} arrows}
\author{Clément Aubert}

\begin{document}
	\maketitle 	
	
		Then $P \r{fb}[\alpha] Q$ denotes $P \r{f}[a] Q$ if $\alpha = a$ and $P \r{b}[a] Q$ 
	
	This package provides macros for 8 different types of arrows: 
	
	\begin{tabular}{| l | l | c | l | c | l |}
		\hline 
		Type & \multicolumn{2}{c|}{Without \(\mapstochar\) char} & \multicolumn{2}{c|}{With \(\mapstochar\) char} \\
		\hline 
		& Arrow & Macro & Arrow & Macro \\
		\hline
		Straight, half tip & $X \r{c}[a] Y$  & \verb|\r{c}[a]| &  $P \pr{c}[a] Q$  & \verb|\pr{c}[a]| \\
		\hline 
		Straight, full tip & $X \r{f}[a][m] Y$ & \verb|\r{f}[a][m]| & $X \pr{f}[a][m] Y$ & \verb|\pr{f}[a][m]|\\
		\hline 
		Squigly, full tip & $X \r{b}[a][m] Y$ & \verb|\r{b}[a][m]| & $X \pr{b}[a][m] Y$ & \verb|\pr{b}[a][m]|\\
		\hline 
		Straight, double tip & $X \r{fb}[a][m] Y$ & \verb|\r{fb}[a][m]| & $X \pr{fb}[a][m] Y$ & \verb|\pr{fb}[a][m]|\\
		\hline
	\end{tabular} 
	
	The second optional argument is not available for the \verb|\r{c}| and \verb|\pr{c}| for \enquote{historical} reasons, but could be added if needed.
	
	It accomodates reasonably well subscripts and superscripts in the labels:
	
	\begin{tabular}{| c | l |}
		\hline 
		Arrow & Macro \\
		\hline 
		$P \pr{c}[\theta_{E}^i] Q$  & \verb|\pr{c}[\theta_{E}^i]| \\
		\hline 
		$X \pr{f}[\theta_{E}^i] Y$ & \verb|\pr{f}[\theta_{E}^i]|\\
		\hline 
		$X \pr{b}[\theta_{E}^i] Y$ & \verb|\pr{b}[\theta_{E}^i]|\\
		\hline 
		$X \pr{fb}[\theta_{E}^i] Y$ & \verb|\pr{fb}[\theta_{E}^i]|\\
		\hline
	\end{tabular} 

The commands \verb|\fbarrowraise| and \verb|\fbarrowheight| (both set by default to -0.3ex) can be redefined to give more room to labels:

\renewcommand{\fbarrowraise}{0.1ex}
\renewcommand{\fbarrowheight}{0.6ex}

	\begin{tabular}{| c | l |}
	\hline 
	Arrow & Macro \\
	\hline 
	$P \pr{c}[\theta_{E}^i] Q$  & \verb|\pr{c}[\theta_{E}^i]| \\
	\hline 
	$X \pr{f}[\theta_{E}^i] Y$ & \verb|\pr{f}[\theta_{E}^i]|\\
	\hline 
	$X \pr{b}[\theta_{E}^i] Y$ & \verb|\pr{b}[\theta_{E}^i]|\\
	\hline 
	$X \pr{fb}[\theta_{E}^i] Y$ & \verb|\pr{fb}[\theta_{E}^i]|\\
	\hline
\end{tabular} 
	
	Note that we can use \verb|bf| or \verb|fb| with the same result.
	
	Those arrows can reasonably be drawn in Ti\emph{k}Z using the following:
	\tikzstyle{f} = [-> %
	] % Style for proved forward arrow 
	\tikzstyle{b} = [decorate, %
	decoration={snake, amplitude=-1pt, segment length=1.5mm, pre=lineto, post length=2pt, pre length=1pt},%
	-{Straight Barb[scale=0.8]} %
	] % Style for proved backward arrow 
	\tikzstyle{fb} = [arrows={- angle 45},%
	-{>[sep=-1pt]>}%
	] % Style for proved forward & backward arrows
	
	
	\tikzstyle{pf} = [{Bar[]}-> %
	] % Style for proved forward arrow 
	\tikzstyle{pb} = [decorate, %
	decoration={snake, amplitude=-1pt, segment length=1.5mm, pre=lineto, post length=2pt, pre length=1pt},%
	{Bar[]}-{Straight Barb[scale=0.8]} %
	] % Style for proved backward arrow 
	\tikzstyle{pfb} = [arrows={- angle 45},%
	{Bar}-{>[sep=-1pt]>}%
	] % Style for proved forward & backward arrows
	
	{\centering
		\tikz{\draw[f] (0,0)   -- (1,0);}
		\tikz{\draw[b] (0,0)   -- (1,0);}
		\tikz{\draw[fb] (0,0)   -- (1,0);}
		\tikz{\draw[pf] (0,0)   -- (1,0);}
		\tikz{\draw[pb] (0,0)   -- (1,0);}
		\tikz{\draw[pfb] (0,0)   -- (1,0);}
	\end{document}
